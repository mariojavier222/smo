...
$(document).ready(function(){
	...
	$("#barraMostrar").click(function(evento){
		...
		$.ajax({
			...
			error: function (XMLHttpRequest, textStatus){
				alert(textStatus);},
			data: {LecID: vLecID, CurID: vCurID, NivID: vNivID, DivID: vDivID, PerID: vPerID, Turno: vTurno, Filtro: vFiltro},
			url: 'mostrarDatosPadresEmail.php',
			success: function(data){ 
				$("#mostrar").html(data);
			}
		});//fin ajax//*/
	...
	$("#barraEnviar").click(function(evento){
		...
		$("input[id^='DNI']").each(function(index){
			...
			if (valorIns==1){
				...
				vPadre = $("#Padre" + i).val();
				vMadre = $("#Madre" + i).val();
				EmailPadre = $("#EmailPadre" + i).val();
				EmailMadre = $("#EmailMadre" + i).val();
				vAlumno = $("#Alumno" + i).val();
				vDeuda = $("#Deuda" + i).val();	
				vMensaje = $("#Mensaje").val();				
				$.ajax({
					type: "POST",
					cache: false,
					async: false,
					error: function (XMLHttpRequest, textStatus){
						alert(textStatus);},
					data: {opcion: "enviarEmailPadres", DNI: vDNI, Padre: vPadre, Madre: vMadre, Alumno: vAlumno, Deuda: vDeuda, Mensaje: vMensaje, EmailPadre: EmailPadre, EmailMadre: EmailMadre},
					url: 'cargarOpciones.php',
					success: function(data){ 
		...
		$("input[id^='DNI']").each(function(index){
			...
			if (valorIns==1){
				...
				$.ajax({
					...
					error: function (XMLHttpRequest, textStatus){
						alert(textStatus);},
					data: {opcion: "actualizarCuotasAlumnoCursado", NivID: vNivID, LecID: vLecID, DNI: vDNI, PerID: vPerID},
					url: 'cargarOpciones.php',
					success: function(data){ 						
						if (data){
	...
	$("#barraAsignarEspecial").click(function(evento){
		evento.preventDefault();
		$("#mostrarErrores").empty();
		iTotal = 1;
		$("input[id^='DNI']").each(function(index){
			...
			vDNI = $("#DNI" + i).val();		
			vPerID = $("#PerID" + i).val();			
			vCMoID = $("#CMoID option:selected").val();
			$.ajax({
				type: "POST",
				cache: false,
				async: false,
				error: function (XMLHttpRequest, textStatus){
					alert(textStatus);},
				data: {opcion: "asignarCuotasEspecial", CMoID: vCMoID, PerID: vPerID},
				url: 'cargarOpciones.php',
				success: function(data){ 						
					if (data){
						$("#mostrarErrores").append(data);							
...
<div id="mostrarCuotas">
	...
	<table width="95%" border="0" align="center" class="borde_recuadro">
      <tr>
        <td colspan="2">		
		<div align="center" class="titulo_noticia"><img src="imagenes/Email.png" width="48" height="48" align="absmiddle" /> Datos de los Padres</div></td>
      </tr>
	   <tr>
         ...
         <td><?php 
			...
			?> 
           <input name="Turno" type="hidden" id="Turno" value="999999" />
             </tr>               
      <tr>
  ...
  <tr>
       ...
       <tr>
         ...
         <td><?php cargarListaDivision("DivID", true);?> 
             ...
             <tr>
         <td class="texto"><div align="right">Filtrar:</div></td>
         <td><select name="Filtro" id="Filtro">
           <option value="1" selected="selected">Todos los alumnos</option>
	    ...
	    <td width="50%" class="texto"><div align="right"><strong>DNI   :</strong></div></td>
          <td>
          <input name="DNI" type="number" class="texto_buscador" id="DNI" size="15" disabled="disabled" />
        *       
      ...
      <tr>
        <td colspan="2" align="center" class="texto"><br /></td>
      </tr>
      <tr>
        <td colspan="2" align="center" class="texto"><button class="botones" id="barraMostrar">
        Cargar Datos</button> <!--<button class="botones" id="barraEnviar">
        Enviar Email</button>   -->      
