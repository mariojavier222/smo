...
$sql = "SELECT * FROM Persona $sqlWhere AND Per_ID NOT IN (SELECT Fam_Per_ID FROM Familia WHERE Fam_FTi_ID = 1) ORDER BY Per_Apellido, Per_Nombre";
$sql = "SELECT * FROM Persona $sqlWhere ORDER BY Per_Apellido, Per_Nombre";
if (!empty($distintos)) $sql = "SELECT DISTINCTROW Per_Apellido FROM Persona $sqlWhere AND Per_ID NOT IN (SELECT Fam_Per_ID FROM Familia WHERE Fam_FTi_ID = 1) ORDER BY Per_Apellido, Per_Nombre";
