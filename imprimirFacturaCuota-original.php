...
$total = $i;
	...
	$pdf->addText(500,$yArriba - 70,$fontSize,$Fac_Numero);
	$pdf->addText(490,$yArriba - 95,$fontSize,$dia);
	$pdf->addText(515,$yArriba - 95,$fontSize,$mes);
	$pdf->addText(535,$yArriba - 95,$fontSize,$anio);
	$pdf->addText(40,$yArriba - 130,$fontSize,$row['Fac_PersonaNombre']);

	
 
	$letrasTotal = new EnLetras();
	$valorLetras = $letrasTotal->ValorEnLetras($row['Fac_ImporteTotal'], "");
	$pdf->addText(200,$yArriba - 150,$fontSize,$valorLetras);
	 ...
	 $sql_1 = "SELECT * FROM FacturaDetalle WHERE FDe_Fac_ID =$FacID";
        ...
        if (mysqli_num_rows($resultado) > 0) {
			...
			while ($row1 = mysqli_fetch_array($resultado)) {
				//Detalle
				$pdf->addText(40,$y,$fontSize,$row1['FDe_Detalle']);
				$pdf->addText(40,$yy,$fontSize,$row1['FDe_Detalle']);
				
				//Importe
				$pdf->addText(480,$y,$fontSize,"$".$row1['FDe_Importe']);
			...
			$fontSize = 10;
			//Importe total
			$pdf->addText(480,$yArriba - 333,$fontSize,"$".$row['Fac_ImporteTotal']);
			...
			if (is_numeric($Fac_ID_Ndec)){
				...
				$pdf->addText(40,$yArriba - 280,$fontSize,"Comprobante asociado ".$InfoFactura);
			...
			//Usuario que cobró
			$fontSize = 8;
			$pdf->addText(40,$yArriba - 315,$fontSize,"Atendido por $Usuario");
