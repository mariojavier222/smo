...
if (!empty($DNI)){
	$fechaHoy = date("d-m-Y");
	$Deuda = Obtener_Deuda_Sistema($DNI);
	if ($Deuda>0){		
	?>
<link href="css/general.css" rel="stylesheet" type="text/css" />

		<br /><div class="borde_alerta" align="center">
		  <p class="texto"><img src="botones/Warning.png" alt="Advertencia" width="24" height="24" align="absbottom" />Al día de la fecha <?php echo $fechaHoy;?> la persona <strong><?php echo "$usuario_nombre $usuario_apellido";?></strong> tiene una deuda vencida de $<?php echo $Deuda;?>.</p></div><br />
	<?php
	}else{
	
	}//*/

	$sql = "SELECT Persona.Per_DNI, Persona.Per_Doc_ID, FTi_ID, Persona.Per_Apellido, Persona.Per_Nombre, Persona.Per_ID FROM
    Familia
...
<div id="listado">	
  ...
  <table width="100%" border="0">
    <tr>
      ...
      <th class="fila_titulo"><div align="left">Tipo de cuota </div></th>
	...
	while ($row_prim = mysqli_fetch_array($result_prim)){
		...
		echo "$foto $parentesco: <strong>$row_prim[Per_Apellido], $row_prim[Per_Nombre]</strong> (DNI: $row_prim[Per_DNI])";?>&nbsp;</td>
	  	</tr>
	...
	$PerID = $row_prim[Per_ID];
	$DocID = $row_prim[Per_Doc_ID];
	$sql = "SELECT * FROM
    CuotaPersona
    INNER JOIN Lectivo 
