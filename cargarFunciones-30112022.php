...
//borrar
  ...
  Apellido y Nombre: <strong><?php echo "$row[Per_Apellido], $row[Per_Nombre]"; ?></strong><br />
                        ...
                        $mostrar_beneficios = "Ciclo ".$nombre_lectivo.': '.$mostrar_beneficios;
                  ...
                  <?php if ($cantH>0){
					 	for ($i=1;$i<=$cantH;$i++){
                            ...
                            //if ($_SESSION['sesion_UsuCaja']==1){
                                $arrayBeneficios = obtenerBeneficioAlumnoLectivo($Lec_ID, $arrarHermanos[$i]['Per_ID']);
                                $total_beneficios = count($arrayBeneficios);
                                $mostrar_beneficios = '';
                                for ($iBen=0; $iBen < $total_beneficios; $iBen++) { 
                                    $mostrar_beneficios .= $arrayBeneficios[$iBen]['Ben_Nombre'].' ('.cfecha($arrayBeneficios[$iBen]['CBe_Desde']).'-'.cfecha($arrayBeneficios[$iBen]['CBe_Hasta']).')<br>';
                                ...
                                if ($_SESSION['sesion_UsuCaja']==1 && !empty($mostrar_beneficios)){
                                    echo "<br><span class='cuota_beneficio'><strong>".$mostrar_beneficios."</strong></span><br>";
                                    //echo $mostrar_beneficios;
                                }
...
function guardarMateriaColegio() {
	...
	$sql = "SET NAMES UTF8;";
	//consulta_mysql_2022($sql,basename(__FILE__),__LINE__);
	$Materia = strtoupper(arreglarCadenaMayuscula(trim(utf8_decode($Materia))));
	$Siglas = strtoupper(arreglarCadenaMayuscula(trim(utf8_decode($_POST['Siglas']))));
	$MatID = $_POST['MatID'];
...
//Eze 15/09
    ...
    $("#NdecBot").click(function(evento){
        ...
        $.ajax({
            ...
            url: 'cargarOpciones.php',
            success: function(data){ 
               jPrompt('Ingrese el código de autorización: <br>' + data , '', 'Busque la posición', function(r){
                if (r){
                    //alert("Bien");
                    cod = data;
                    $.ajax({
...
function editarConfiguracionCuotas()
{
    ...
    //Mario. 23112021
    if (empty($_POST['CMo_2do_Vencimiento'])) $CMo_2do_Vencimiento='0000-00-00';
	if (empty($_POST['CMo_3er_Vencimiento'])) $CMo_3er_Vencimiento='0000-00-00';
...
function GuardarPlanPago(){
	...
	$result = consulta_mysql_2022($sql,basename(__FILE__),__LINE__);
	 ...
	 if (consulta_mysql_2022($sql,basename(__FILE__),__LINE__)){
	 	$Cuo_Cancela_PPa_ID = mysql_insert_id();
	 }else {
