...
$(document).ready(function(){
	...
	$("#barraMostrar").click(function(evento){
		...
		$.ajax({
			...
			error: function (XMLHttpRequest, textStatus){
				alert(textStatus);},
			data: {LecID: vLecID, CurID: vCurID, NivID: vNivID, DivID: vDivID, PerID: vPerID, Turno: vTurno},
		...
		$("input[id^='DNI']").each(function(index){
			...
			//if (valorIns==1){
				...
				$.ajax({
					...
					error: function (XMLHttpRequest, textStatus){
						alert(textStatus);},
					data: {opcion: "actualizarCuotasAlumnoCursado", NivID: vNivID, LecID: vLecID, DNI: vDNI, PerID: vPerID, masivo:masivo},
					url: 'cargarOpciones.php',
					success: function(data){ 						
						if (data){
	...
	$("#barraAsignarMensual").click(function(evento){
		evento.preventDefault();
		$("#mostrarErrores").empty();
		iTotal = 1;
		masivo = '<?php echo date("Ymd-His");?>';
		$("input[id^='DNI']").each(function(index){
			...
			//if (valorIns==1){
				...
				$.ajax({
					...
					error: function (XMLHttpRequest, textStatus){
						alert(textStatus);},
					data: {opcion: "actualizarCuotasAlumnoCursadoMensual", NivID: vNivID, LecID: vLecID, DNI: vDNI, PerID: vPerID, masivo:masivo},
					url: 'cargarOpciones.php',
					success: function(data){ 						
						if (data){
							$("#mostrarErrores").append(data);							
...
<div id="mostrarCuotas">
  ...
  <tr>
	  ...
	  <tr>
	    <td align="right" class="texto">Configuraci&oacute;n de Cuotas:</td>
	    <td><?php //Obtener_LectivoActual($LecID, $Lec_Nombre);
		$LecID = gLectivoActual($Lec_Nombre);
		cargarListaConfigCuota($LecID, "CMoID");?></td> 
      </tr>
      <tr>
        ...
        Comenzar Verificaci&oacute;n</button>
          <button class="botones" id="barraAsignar">Asignar Cuotas (Ojo!)</button>
          <button class="botones" id="barraAsignarMensual">Asignar Cuota MENSUAL</button>
          <button class="botones" id="barraAsignarEspecial">Asignar Configuraci&oacute;n Cuotas</button>
