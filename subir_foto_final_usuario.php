...
if (is_numeric($Usuario)){
	$DNI = substr("000000000".$Usuario,-9);
	$DNI = $DocID.$DNI;
}else{
	$DNI = $Usuario;
}
...
$("#botContinuar").click(function(evento){
	evento.preventDefault();
	$.ajax({
			  type: "POST",
			  cache: false,
			  async: false,
			  url: 'cargarSubirFotoPersona.php',
...
<p class="texto"><button id="botContinuar">Continuar</button></p>
...
function subirFoto($archivo, $path, $ancho, $marcar=false){
	...
	if ($handle->uploaded) {		  
		...
		if ($marcar){
		  $handle->image_text 			= 'www.docmedia.com.ar';
