...
function actualizarCuotasAlumnoCursadoMensual($PerID, $LecID, $NivID, $masivo){
	...
	guardarAsignacionCuotaMensual($PerID, $LecID, $NivID, $masivo);	
...
function guardarAsignacionCuotaMensual($Cuo_Per_ID, $Cuo_Lec_ID, $Cuo_Niv_ID, $masivo='') {
	...
	$sql = "SELECT * FROM CuotaModelo WHERE CMo_Lec_ID = $Cuo_Lec_ID AND CMo_Niv_ID = $Cuo_Niv_ID AND CMo_Especial=0 AND CMo_CTi_ID=2 ORDER BY CMo_Lec_ID, CMo_Niv_ID, CMo_CTi_ID, CMo_Alt_ID, CMo_Numero";
	$result = consulta_mysql_2022($sql,basename(__FILE__),__LINE__);
