...
$total = $i;
	...
	$pdf->setLineStyle(1,'','',array(1,0));
	$datacreator = array (
                    'Title'=>"CERTIFICADO DE LIBRE DE DEUDA",
                    'Subject'=>'CERTIFICADO DE LIBRE DE DEUDA',
                    'Author'=>'',
	...
	//logo del colegio
	$pdf->addJpegFromFile($ruta_images,250,$yArriba - 40,80,0);
	...
	//$pdf->line(20,$yArriba-50,575,$yArriba-50);
	//$pdf->addText(0,$yArriba - 50,12,"__________________________________________________________________________________");
	$pdf->ezSetY(700);
	$fontSize = 12;
	$pdf->ezText("\n________________________________________________________________________________",$fontSize, array('justification'=>'center'));
	...
	$fontSize = 12;	
	$pdf->ezText("\nLa Administración del Colegio ".utf8_decode(COLEGIO_NOMBRE).", certifica que <b>$Alumno</b>, D.N.I. Nº <b>$DNI</b>, no registra deuda alguna con esta Institución al día de la fecha.-\n", $fontSize, array('justification'=>'full'));	

	$pdf->ezText("\nA solicitud del interesado, y para ser presentado ante quien corresponda, se extiende el presente en Capital, San Juan, a los <b>$textoDia</b> días del mes de <b>$textoMes</b> de <b>$textoAnio</b>.-\n", $fontSize, array('justification'=>'full'));	
	$fontSize = 12;
	$pdf->ezText("\n________________________________________________________________________________",$fontSize, array('justification'=>'center'));
	$fontSize = 10;
	$pdf->ezText(utf8_decode(COLEGIO_DOMICILIO)."\nTeléfono: ".COLEGIO_TELEFONO,$fontSize, array('justification'=>'center'));
