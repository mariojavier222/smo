...
$sqel = "SELECT * FROM Persona WHERE Per_DNI = '$DNI'";
$resulta = consulta_mysql_2022($sqel,basename(__FILE__),__LINE__);
$roww = mysqli_fetch_array($resulta);
$PerID = $roww[Per_ID];

$sql = "SELECT * FROM Requisito
    INNER JOIN Colegio_Nivel 
        ON (Requisito.Req_Niv_ID = Colegio_Nivel.Niv_ID)ORDER BY Colegio_Nivel.Niv_ID";
...
<div id="listado">	
 <table width="100%" border="0">
	...
	while ($row = mysqli_fetch_array($result)){		
		...
		$NivID = $row[Niv_ID];
		$sql = "SELECT * FROM RequisitoPresentado WHERE Pre_Niv_ID = '$NivID' AND Pre_Per_ID = '$PerID' AND Pre_Req_ID = '$row[Req_ID]'";
