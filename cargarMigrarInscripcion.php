...
$(document).ready(function(){
	...
	$("#barraCambiar").click(function(evento){
		...
		$.ajax({
			...
			error: function (XMLHttpRequest, textStatus){
				alert(textStatus);},
			data: {opcion: "migrarInscripcionesAlumnos", LecID: vLecID, CurID: vCurID, NivID: vNivID, DivID: vDivID, NivNuevoID: vNivNuevoID, LecNuevoID: vLecNuevoID, CurNuevoID: vCurNuevoID, DivNuevoID: vDivNuevoID, Inscriptos: vInscriptos},
			url: 'cargarOpciones.php',
			success: function(data){ 
	...
	$("#NivID").change(function () {
		$("#NivID option:selected").each(function () {			
			NivID = $("#NivID").val();
			llenarCursoTurno(NivID, "CurID");
		});
    });
    $("#NivNuevoID").change(function () {
		$("#NivNuevoID option:selected").each(function () {			
			NivID = $("#NivNuevoID").val();
			llenarCursoTurno(NivID, "CurNuevoID");
		});
    });
	
    function llenarCursoTurno(NivID, nombre){
		$("#cargando").show();
		//NivID = $("#NivID").val();
		TurID = 999999;
		$.ajax({
	...
	<table width="95%" border="0" align="center" class="borde_recuadro">
      <tr>
		...
		<div align="center" class="titulo_noticia">
		  <p><img src="imagenes/application32.png" width="32" height="32" align="absmiddle" /> Migrar Inscripciones de los Alumnos<br />
      ...
      <tr>
        <td class="texto"><table width="100%" border="0" cellspacing="1" cellpadding="1">
          <tr>
    <td colspan="2" align="center" class="texto"><strong>Datos del Lectivo Actual</strong></td>
    </tr>
          <tr>
...
//			cargarListaLectivoInscripcion("LecID", $UniID);
  ...
  <tr>
    <td colspan="2" align="center" class="texto"><strong>Datos del Nuevo Ciclo Lectivo</strong></td>
    </tr>
    <tr>
         <td width="50%" class="texto"><div align="right">Ciclo Lectivo:</div></td>
         <td><?php 
		 	$UniID = $_SESSION['sesion_UniID'];
			//echo $UniID;
			cargarListaLectivoInscripcion("LecNuevoID", $UniID);
			//cargarListaLectivo("LecNuevoID");
			?> 
    ...
    <td><?php cargarListaCursos("CurNuevoID", true);?> 
      ...
      <tr>
        <td colspan="2" align="center" class="texto"><button id="barraCambiar" class="botones">
          Migrar Inscripciones</button></td>
      </tr>
        </table></td>
      </tr>
      <tr>
