 ...
 <table width="100%" border="0" cellspacing="0" class="display" id="listadoTabla">
    <thead>
        <tr>
          ...
          <th align="center" class="fila_titulo">&nbsp;</th>
          <?php
		  $sql = "SELECT * FROM Libro WHERE Lib_Cur_ID = $CurID";
		  $result2 = consulta_mysql_2022($sql,basename(__FILE__),__LINE__);
		  if (mysqli_num_rows($result2)){
			  $cantGrilla = 0;
			  while ($row2 = mysqli_fetch_array($result2)){ 
				  $cantGrilla++;
				  $mGrilla[$cantGrilla] = $row2[Lib_ID];
				  
				  ?>
				  <th align="center" class="fila_titulo"><?php echo $row2[Lib_Nombre];?></th>
				  <?php
...
/*	$div = $row[Div_Nombre];
	...
	<tr id="fila<?php echo $i;?>" >
      ...
      <td style="<?php echo $claseFila;?>"><?php echo "$row[Per_Apellido], $row[Per_Nombre]";?></td>    
	   ...
	   <td style="border-right-width: 1px;	border-bottom-width: 1px;	border-right-style: solid;	border-bottom-style: solid;	border-right-color: #000;	border-bottom-color: #000;"></td>
	   <?php
          for($m=1;$m<=$cantGrilla;$m++){
		  ?>
          <td style="<?php echo $claseGrilla;?>" align="center"><?php echo tieneLibroPersona($row[Per_ID], $mGrilla[$m])?></td>
          <?php
		  }//fin for
		  ?>
	...
	?>  
     ...
     <tfoot>
        <tr>
          <th colspan="11" class="fila_titulo"></th>
        </tr>
        </tfoot>
</table>
</div>
