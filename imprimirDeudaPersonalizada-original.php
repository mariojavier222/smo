...
if ($Deuda>0){	
	...
	$pdf->ezText("<b><c:uline>ESTADO DE DEUDA</c:uline></b>", $fontSize, array('justification'=>'center'));
	$fontSize = 10;
	...
	$texto = "<b>Estimados Padres</b> $hijoArticulo $hijoTitulo <b>$PPa_Alumno</b> de <b>$Curso de $Nivel</b>. Queremos informarle que nos encontramos implementando un nuevo Sistema Informático para el Control de las Cuotas. Debido a esto, le pedimos que controle la situación económica de su hij$hijoVocal, que al día de la fecha, <b>$dia de $mes de $anio</b>, posee impagas y vencidas las siguientes cuotas según se detallan:\n";
	...
	$pdf->ezText($texto, $fontSize, array('justification'=>'full'));
	$texto = "<b><i>LA ADMINISTRACIÓN</i> - Colegio Parroquial Santa Lucía</b>\nHorario de atención: Lun a Vie de 08 a 14 hs. y de 15 a 18:30 hs.";
	$pdf->ezText($texto, $fontSize, array('justification'=>'right'));
	$pdf->ezText("<b>- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -</b>\n", 14, array('justification'=>'full'));
	
	if ($i%3==0) $pdf->ezNewPage();
}//fin if deuda
}//fin while
