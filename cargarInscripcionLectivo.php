...
$(document).ready(function(){
				...
				$.post("cargarOpciones.php",{opcion: 'buscarDNI', DNI: vDNI}, function(data){
					...
					$.post("cargarOpciones.php",{opcion: 'buscarPerID', DNI: vDNI}, function(data){
						...
						if (vPerID>0){
							...
							//cargarPreinscCurso(vLecID,vLegID);
							...
							$.post("cargarOpciones.php",{opcion: 'buscarDatosInscripcionLectivo', LegID: vLegID, LecID: vLecID, NivColegio: vNivColegio}, function(data){
								...
								if (data!="{}"){
									...
									controlarNivel(vCurID);
									datosRegistroInscripcion(vLegID,obj.Ins_Lec_ID,obj.Ins_Niv_ID,obj.Ins_Usu_ID);
									
									$("#NivID").attr("disabled","disabled");
									$("#LecID").attr("disabled","disabled");
									$("#CurID").attr("disabled","disabled");
									$("#DivID").attr("disabled","disabled");								
									
								}else {
									$("#CurID").val(-1);
									controlarNivel($("#CurID").val());
									$("#DivID").val(-1);
	...
	function datosRegistroInscripcion(Leg_ID,Lec_ID,Niv_ID,Usu_ID){
		$.post("cargarOpcionesInscripcion.php", { opcion: 'datosRegistroInscripcion', Leg_ID:Leg_ID,Lec_ID:Lec_ID,Niv_ID:Niv_ID,Usu_ID:Usu_ID}, function(data){
				//alert(data);
   ...
   $("#LecID").change(function () {
   	...
   	function cargarPreinscCurso(LecID,LegID){
		$.ajax({
            type: "POST",
            cache: false,
            async: false,			
            url: 'cargarPreinscCurso.php',
            data: {LecID: LecID, LegID: LegID},
            success: function (data){
            	//alert(data);
                $("#Preinsc").html(data);
                $("#cargando").hide();
	  ...
	  <table width="80%" border="0" align="center" class="borde_recuadro">
        ...
        *       
        ...
        <td><div class="ui-state-error ui-corner-all" id="noTieneLegajo" style="font-size:14px"></div>
        	...
        	<div class="titulo_noticia" id="Preinsc" name="Preinsc" style="font-size:15px; background-color:#FFCC00;"></div>
       </tr>
       <tr>
       ...
       <tr>
         ...
         <td colspan="2"><?php cargarListaNivelUsuarioAcotado("NivID",false,true,$UsuID);?> 
	   </tr>	
       <tr>
         <td class="texto"><div align="right">Datos de Registro:</div></td>
         <td colspan="2"><div id='datos_registro' name='datos_registro' style=" font-size:14px; font-weight: bold;"></div> 
	   </tr>	
