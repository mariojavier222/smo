...
//sleep(3);
	...
	$sql = "SELECT DISTINCTROW Persona.Per_ID
    , Persona.Per_DNI
    ...
    , Legajo.Leg_Numero
    , Carrera.Car_Nombre
    , Plan.Pla_Nombre FROM
   InscripcionMateria
    INNER JOIN Inscripcion 
        ON (InscripcionMateria.IMa_Leg_ID = Inscripcion.Ins_Leg_ID) AND (InscripcionMateria.IMa_Car_ID = Inscripcion.Ins_Car_ID) AND (InscripcionMateria.IMa_Pla_ID = Inscripcion.Ins_Pla_ID) AND (InscripcionMateria.IMa_Lec_ID = Inscripcion.Ins_Lec_ID)
    INNER JOIN Legajo 
        ON (Inscripcion.Ins_Leg_ID = Legajo.Leg_ID)
    INNER JOIN Division 
        ON (InscripcionMateria.IMa_Div_ID = Division.Div_ID)
    INNER JOIN Materia 
        ON (InscripcionMateria.IMa_Car_ID = Materia.Mat_Car_ID) AND (InscripcionMateria.IMa_Mat_ID = Materia.Mat_ID)
    INNER JOIN Plan 
        ON (InscripcionMateria.IMa_Pla_ID = Plan.Pla_ID)
    INNER JOIN Persona 
...
WHERE (Ins_Lec_ID = $LecID AND Leg_Sed_ID = $SedID AND Leg_Baja = 0 ";
