...
$UsuID = 1;
$Lec_ID = 18;
$sql = "SELECT Per_ID, UPPER(Per_Apellido)AS Per_Apellido, Per_Nombre, Cur_Siglas, Div_Siglas FROM
    Colegio_Inscripcion
    INNER JOIN Legajo
	...
	WHERE Ins_Lec_ID = $Lec_ID AND Ins_Niv_ID = 3 ORDER BY Ins_Cur_ID, Ins_Div_ID"; //echo "$sql<br>";
...
if (mysqli_num_rows($result)>0){
	...
	while ($row = mysqli_fetch_array($result)){
		...
		$cuota = ".".$row['Cur_Siglas']."-".$row['Div_Siglas']."[MAR/19]";
