...
WHERE (Colegio_Inscripcion.Ins_Lec_ID = 20 AND 
	Ins_Div_ID>0  AND 
	Leg_Sed_ID = $SedID AND 
	Per_Baja = 0 AND 
	Leg_Baja = 0 AND
	Cuo_Pagado = 0 AND 
	Cuo_Cancelado = 0 AND 
	Cuo_Anulado = 0 AND 
	(Cuo_1er_Vencimiento >= '2000-01-01' AND Cuo_1er_Vencimiento <= '2021-12-31')";
	...
	$sql.=") GROUP BY Per_ID ORDER BY Niv_ID, Cur_ID, Div_ID, Persona.Per_Sexo, Per_Apellido, Per_Nombre;";
 ...
 <table width="100%" border="0" id="listadoTabla" class="display">
    <thead>
        <tr>
          ...
          <th align="center" class="fila_titulo">Recargos</th>
...
/*	$div = $row[Div_Nombre];
	...
	<tr class="gradeA" id="fila<?php echo $i;?>" height="40px">
      ...
      <td align="center">     
      <a href="#" onclick="buscarCuotasImpagas2('<?php echo $PerID ?>')" style="cursor:pointer">
      	<?php echo "$".$Deuda; ?></a></td>
      	<td><?php echo $Recargos;?></td>
      	<td><?php echo $Deuda + $Recargos;?></td>
    </tr>
...
$Adeudado = $TotalDeuda + $TotalRecargos;
echo " - <strong>Total Deuda: $ $TotalDeuda - Total Recargos: $ $TotalRecargos - Total Adeudado: $ $Adeudado</strong>";
?></div>
<br /><br /></fieldset>
