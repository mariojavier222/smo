...
//tipo de factura: 1 recibo, 2 factura, 3 nota de crédito
$tituloComprobante='';
$TipoComprobante=$row['Fac_FTi_ID'];
if ($TipoComprobante==1) $tituloComprobante="<b>RECIBO DE COBRANZAS</b>";
if ($TipoComprobante==2) $tituloComprobante="<b>RECIBO DE COBRANZAS</b>";
if ($TipoComprobante==3) $tituloComprobante="<b>NOTA DE CRÉDITO</b>";
...
if ($imprimir){  
	...
	$pdf->addText(370,$yArriba,11,$tituloComprobante);
	$pdf->addText(370,$yArriba - 20,11,"<b>N°:</b> C".$row['Fac_Sucursal']."-".$row['Fac_Numero']);
    ...
    if (mysqli_num_rows($resultado) > 0) {
		...
		while ($row1 = mysqli_fetch_array($resultado)) {
			...
			if (mysqli_num_rows($resultado) >= 15) {//achico la letra
				//Detalle
				$pdf->addText(40,$y,8,utf8_decode($row1['FDe_Detalle']));

				//Importe
				$pdf->addText(450,$y,8,"$".number_format($row1['FDe_Importe'],2,',','.'));
				$cambia=1;
			}else {
				//Detalle
		...
		if ($row['Fac_Pagada']==1 && $TipoComprobante<3){
	...
	if (is_numeric($Fac_ID_Ndec)){
		...
		$pdf->addText(100,$yArriba - 270,$fontSize,"Comprobante asociado ".$InfoFactura);
	...
	//Importe total
	$pdf->addText(380,$yArriba - 333,12,"<b>TOTAL:</b> $".number_format($row['Fac_ImporteTotal'],2,',','.'));
	
}//del if si viene el DNI imprimo
else {
	//muestro un mensaje que no se puede mostrar esta boleta porque faltan datos
	$pdf->addText(40,770,10,"<b>NO SE PUEDE IMPRIMIR EL COMPROBANTE</b>");
	$pdf->addText(50,750,9,"Motivo: El número $Sucursal-$Numero no es válido");
