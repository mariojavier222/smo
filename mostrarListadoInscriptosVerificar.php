...
//		if ($Turno!=999999) $sql.=" AND Cur_Turno = '$Turno' ";
	...
	if (!empty($Fecha)) {
		...
		$sql.=" AND Colegio_Inscripcion.Ins_Fecha = '$Fecha'";
	}
 ...
 <table width="100%" border="0" id="listadoTabla" class="display">
    <thead>
        <tr>
          ...
          <th align="center" class="fila_titulo">Inscr. Verif.</th>
        </tr>
       </thead>
       <tbody>
	<?php $i=0;
/*	$div = $row[Div_Nombre];
	...
	<tr class="<?php echo $clase;?>" id="fila<?php echo $i;?>" height="40px" title="<?php echo $title;?>">
      <td align="center"><?php echo $row['Per_DNI'];//$row[Leg_Numero];?>
        <input type="hidden" id="DNI<?php echo $i;?>" name="DNI<?php echo $i;?>" value="<?php echo $row['Per_DNI'];?>" />
        <input type="hidden" id="Leg<?php echo $i;?>" name="Leg<?php echo $i;?>" value="<?php echo $row['Leg_ID'];?>" />
        <input type="hidden" id="Lec<?php echo $i;?>" name="Lec<?php echo $i;?>" value="<?php echo $row['Ins_Lec_ID'];?>" />
        <input type="hidden" id="PerID<?php echo $i;?>" name="PerID<?php echo $i;?>" value="<?php echo $row['Per_ID'];?>" />
        <input type="hidden" id="Niv<?php echo $i;?>" name="Niv<?php echo $i;?>" value="<?php echo $row['Ins_Niv_ID'];?>" />
        <input type="hidden" id="Inscrip<?php echo $i;?>" name="Inscrip<?php echo $i;?>" value="<?php echo $row['Ins_Provisoria'];?>" /></td>
