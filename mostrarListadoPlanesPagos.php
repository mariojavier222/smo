...
WHERE (Colegio_Inscripcion.Ins_Lec_ID = $LecID AND Ins_Div_ID>0  AND Leg_Sed_ID = $SedID AND Per_Baja = 0 AND Leg_Baja = 0 ";
    ...
    INNER JOIN Colegio_Nivel 
        ON (Colegio_Inscripcion.Ins_Niv_ID = Colegio_Nivel.Niv_ID)
ORDER BY Niv_ID, Cur_ID, Div_ID, Persona.Per_Sexo, Per_Apellido, Per_Nombre;";
 ...
 <table width="100%" border="0" id="listadoTabla" class="display">
    <thead>
        <tr>
          ...
          <th align="center" class="fila_titulo">Acta</th>
          <th align="center" class="fila_titulo">Fecha </th>
          <th align="center" class="fila_titulo">Deuda Plan Pago</th>
...
/*	$div = $row[Div_Nombre];
	...
	while ($row = mysqli_fetch_array($result)){		
		...
		//$tieneBeneficio = obtenerBeneficioAlumno($LecID, $PerID);
		if ($Deuda>0){
		//if ($tieneBeneficio){	
			$iDeuda++;
	...
	<tr class="gradeA" id="fila<?php echo $i;?>" height="40px">
      ...
      <td><?php echo $row['Niv_Nombre']." ".$row['Cur_Siglas']." ".$row['Div_Siglas'];?></td>
      <td align="center"><a href="imprimirPlanPago.php?id=<?php echo $row['PPa_ID'];?>" target="_blank" title="Imprimir Acta"><img src="imagenes/printer.png" width="32" height="32" /></a></td>
      <td><?php echo cfecha($row['PPa_Fecha']);?></td>
      <td>$<?php echo $row['PPa_DeudaTotal'];?></td>
      <td align="center">
