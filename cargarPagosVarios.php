...
$(document).ready(function(){
	...
	$("button[id^='pago']").click(function(evento){							  
			...
			if(vobservaciones == ''){
				jAlert("Ingrese Observaciones");
				return;
				}
			if(isNaN(vimporte)){
				jAlert("Ingrese Importe");
				return;
				}	
			vError = false;
	        vTexto_Error = '';
                ...
                if (vimporte > vtotal){
					...
					if(vError) {
					jAlert(vTexto_Error,"Existen datos incorrectos");
					return;
					}
			$.ajax({
                type: "POST",
                ...
                data: {opcion: 'guardarPagosVarios', forma: vforma, caja: vcaja, total: vtotal, observaciones: vobservaciones , valor: vimporte},
                success: function (data){
                    //$("#principal").html(data);
                    //mostrarAlerta(data);
					$("#listado").show();
                    jAlert("Se guardaron correctamente los cambios." + data, "Confirmación")
					$('#barraPagVarios').click();
                }
            });//fin ajax
        });	
...
<div id="listado" class="page-break">    
        <div align="center" class="titulo_noticia">Listado de Pagos Varios</div>
        <?php
          ...
          <tbody>
            <?php
                    ...
                    <tr style="background-color: gainsboro;">
                      ...
                      <td>
                      <button class="botones" id="pago<?php echo $i?>">Pagar</button></td>      
                    </tr>	
            <?php
                    }//fin while
                    ?>
